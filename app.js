(() => {
  ...
  const displayViewport = document.getElementById('displayViewport');
  ...
  function fitResultExpression() {
    if (!justEvaluated || !lastExpression) return;
    expressionLine.style.fontSize = '28px';
    const available = expressionLine.clientWidth;
    if (expressionLine.scrollWidth > available) expressionLine.style.fontSize = '22px';
  }

  // iOS 26 uses one clear size reduction for the live calculation.
  // After that single step, the line stays at the compact size and the viewport
  // follows the newest characters instead of repeatedly shrinking the text.
  ...
  // Native iOS 26 Liquid Glass controls visibly deform/highlight on touch.
  // Keep the flash on screen for a few frames so even very fast taps are visible.
  document.querySelectorAll('.key, .round-control').forEach((button) => {
    button.addEventListener('pointerdown', () => pressOn(button));
    button.addEventListener('pointerup', () => pressOff(button));
    button.addEventListener('pointercancel', () => pressOff(button, 0));
    button.addEventListener('pointerleave', (event) => {
      if (event.buttons) pressOff(button, 0);
    });
  });
